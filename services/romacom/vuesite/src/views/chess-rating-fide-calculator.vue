  ...
  <div class="page-container">
    ...
    <div class="main-content">
      ...
      <div class="page-header">
        ...
        <p class="page-subtitle">Калькулятор рейтинга для шахматных матчей</p>

        <!-- Селектор системы рейтинга -->
        <div class="system-selector">
          <div class="selector-buttons">
            <button
              @click="ratingSystem = 'fide'"
              :class="['system-button', { active: ratingSystem === 'fide' }]"
            >
              ФИДЕ
            </button>
            <button
              @click="ratingSystem = 'fsr'"
              :class="['system-button', { active: ratingSystem === 'fsr' }]"
            >
              ФШР
            </button>
          </div>
          <p class="system-description">
            Выбранная система: <strong>{{ getSystemName(ratingSystem) }}</strong>
          </p>
        </div>
      </div>
      ...
      <div class="content-card">
          ...
          <div class="opponents-content">
            ...
            <div v-if="opponents.length > 0" class="calculate-section">
              ...
              >
                ...
                <span v-else>Рассчитать {{ getSystemName(ratingSystem) }} ({{ validOpponents.length }}/{{ opponents.length }})</span>
              </button>
            </div>
          </div>
        </div>
        ...
        <div v-if="calculationResults" class="results-section">
          <div class="section-header">
            ...
            <h2 class="section-title">Результаты {{ getSystemName(ratingSystem) }}</h2>
          </div>
          <div class="results-grid">
            ...
            <div class="results-summary">
              ...
              <div class="summary-item">
                ...
                <span :class="['summary-value', 'change-value', getChangeClass(calculationResults.totalChange)]">
                  {{ formatChange(calculationResults.totalChange) }}
                </span>
              </div>
              <div class="summary-item">
            ...
            <div class="matches-details">
              ...
              <div class="matches-list">
                ...
                >
                  ...
                  <span :class="['match-result', getResultClass(match.result)]">
                    {{ getResultText(match.result) }}
                  </span>
                  <span :class="['match-change', getRatingChangeClass(match.ratingChange)]">
                    {{ formatChange(match.ratingChange) }}
                  </span>
                </div>
              </div>
            </div>
...
const ratingSystem = ref('fide') // 'fide' или 'fsr'
...
const calculateAllMatches = async () => {
  ...
  if (ratingSystem.value === 'fide') {
    // Подготовка данных для ФИДЕ
    const enemies = {}
    ...
    endpoint = '/apito/chess/calculate-rating-fide/'
  } else {
    // Подготовка данных для ФШР
    requestData = {
      hero_rating: initialRating,
      opponents_ratings: validOpponents.value.map(opponent => opponent.rating),
      results: validOpponents.value.map(opponent => getScoreValue(opponent.result))
    }
    endpoint = '/apito/chess/calculate-rating-fcr/'
  }

  try {
    console.log('Отправляем запрос:', requestData) // Для отладки
    ...
    console.log('Получен ответ от сервера:', data) // Для отладки

    // Более надежная обработка ответа от сервера
    let newRating

    // Проверяем разные возможные форматы ответа
    if (typeof data.result === 'number') {
      newRating = data.result
    } else if (typeof data.new_rating === 'number') {
    ...
    const matches = validOpponents.value.map((opponent, index) => ({
      opponentRating: opponent.rating,
      result: opponent.result,
      // Если сервер возвращает детали по матчам, используем их
      ratingChange: data.matches && data.matches[index]
        ? data.matches[index].rating_change || data.matches[index].change || 0
        : Math.round(totalChange / validOpponents.value.length) // Примерное распределение
    }))
    ...
    console.log('Результаты расчета:', calculationResults.value) // Для отладки

  } catch (error) {
    console.error('Error calculating rating:', error)
    alert(`Ошибка при расчёте рейтинга ${getSystemName(ratingSystem.value)}. Проверьте подключение к серверу.`)
  } finally {
